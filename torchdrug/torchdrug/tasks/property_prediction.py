...
class PropertyPrediction(tasks.Task, core.Configurable):
    ...
    Parameters:
        ...
        hidden_dims (list, optional): hidden dimensions of output layer
        conv_output (bool, optional): whether to use convolution in output layer or not
    """
    ...
    def __init__(self, model, task=(), criterion="mse", metric=("mae", "rmse"), num_mlp_layer=1,
                 ...
                 graph_construction_model=None, verbose=0, hidden_dims=None, conv_output=False):
    ...
    def preprocess(self, train_set, valid_set, test_set):
        ...
        if self.conv_output:
            kernel_size = 7
            self.conv = nn.Conv1d(1, 1, kernel_size, padding=3, padding_mode='zeros')
    ...
    def make_pesudo_hidden(self):
        '''
        Make pesudo hidden layers to load model from NodePrediction task
        '''
        num_class = 3
        hidden_dims = [self.model.output_dim] * (self.num_mlp_layer - 1)
        self.tmp_mlp = self.mlp
        self.mlp = layers.MLP(self.model.output_dim, hidden_dims + [num_class])
        self.tmp_mean = self.mean
        self.tmp_std = self.std
        self.register_buffer("mean", torch.as_tensor(0, dtype=torch.float))
        self.register_buffer("std", torch.as_tensor(1, dtype=torch.float))

    def replace_pesudo_hidden(self):
        '''
        Replace pesude hidden layers with task-specific layers
        '''
        self.mlp = self.tmp_mlp
        self.register_buffer("mean", self.tmp_mean)
        self.register_buffer("std", self.tmp_std)
    ...
    def predict(self, batch, all_loss=None, metric=None):
        ...
        pred = []
        if self.conv_output:
            conv_output = self.conv(output["graph_feature"].unsqueeze(1)).squeeze(1)
            pred = self.mlp(conv_output)
        else:
